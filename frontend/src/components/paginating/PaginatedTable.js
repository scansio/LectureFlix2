...
import Reblend, { SharedConfig, useEffect, useRef, useState } from "reblendjs";
import {
  Row,
  Col,
  Table,
  ...
  out: (rowData) => {
    ...
    return statusCode == ACTIVE ? (
      ...
      <b className="text-warning">
        Pending <Spinner animation="grow" variant="danger" size="sm" />
      </b>
    ) : (
...
export const booleanTransform = {
  out: (status) => {
    return status ? (
      ...
      <b className="text-black">False</b>
    );
  },
  in: (statusString) => {
    const sst = statusString.toLowerCase();
...
function PaginatedTable(props) {
  ...
  const [numEqualityValue, setNumEqualityValue] = useState(
    numEquality[0].value
  );
  ...
  useEffect(() => {
    ...
    const defaultSearchKey = props.primaryKey
      ? props.primaryKey
      : keys[0] || null;
    const defaultSortOrder = props.sortOrder || ASCENDING;
    const defaultInitQuery = props.forCurrentUser
      ? { ...{ uid: uidRef.current }, ...props.query }
      : props.query;
    ...
    if (trimmedValue && trimmedValue !== "") {
      newQuery[searchKey] =
        ...
        isSelectedANumber() && numEqualityValue !== numEquality[0].value
          ? {
              [`$${numEqualityValue}`]: checkInputTransform(
                searchValue?.trim()
              ),
            }
  ...
  function isSelectedANumber() {
    return (
      fields && fields[searchKey]?.type == Number && !fields[searchKey]?.virtual
    );
  }
  ...
  function computeValue(result, field, index) {
    const val =
      fields[field]?.type == Boolean
        ? booleanTransform.out(result)
        : checkOutputTransform(field, index);
    if (fields[field]?.virtual) {
      return val;
    } else {
      return props.noDoubleClick ? (
        val
      ) : (
        <DoubleClickCopy noClickOpen={props.noClickOpen}>
          {val}{" "}
        </DoubleClickCopy>
      );
    }
                ...
                >
                  {fieldKeys.map((field, i) =>
                    ...
                    fieldValues[i].virtual ? null : (
                      <option
                        value={fieldValues[i].name}
                        key={fieldValues[i].name}
                      >
                        {fieldValues[i].name}
                      </option>
                    )
      ...
      <Row>
        <Col sm="12">
          {props.type == "card" ? (
            <>
              {loadingError ? (
                <div>Could not load the data</div>
              ) : loading ? (
                <Spinner variant="primary" />
              ) : null}
              <div
                className={
                  props.cardNotCentered ? "s-start-grid" : "s-grid-justify"
                }
              >
                {results?.map((result, resultIndex) => (
                  <>{props?.cardView(result, resultIndex)}</>
                ))}
              </div>
            </>
          ) : (
            <>
              ...
              >
                <thead
                  style={props?.style?.theadStyle || {}}
                  className={`${props?.className?.theadClass || ""}`}
                >
                  {/* Header row */}
                  <tr>
                      ...
                      <th
                        ...
                        onClick={(e) =>
                          ...
                          (fieldKeys[i] !== sortKey
                            ...
                            : setSortOrder(
                                sortOrder == sortingOrder[0].value
                                  ? sortingOrder[1].value
                ...
                <tbody
                  style={props?.style?.tbodyStyle || {}}
                  className={`${props?.className?.tbodyClass || ""}`}
                >
                  {results?.map((result, resultIndex) => {
                    // Row
                    const rowOptions =
                      typeof props?.rowOptions === "function"
                        ? props.rowOptions(result)
                        : props.rowOptions;
